...
pub struct QrArtifact {
    ...
    pub audio_ok: bool,
    /// 扫码直达的分享页 URL（腾讯云 COS 落地页 + 作品数据片段）
...
pub fn default_download_url() -> String {
    std::env::var("FINGERTIP_DOWNLOAD_URL").unwrap_or_else(|_| {
        "https://github.com/Nanyou711/FingerTip/releases/latest".to_string()
    })
}

/// 扫码落地页永久地址（腾讯云 COS 静态托管 docs/landing.html）。
///
...
pub fn landing_page_url() -> String {
    std::env::var("FINGERTIP_LANDING_PAGE_URL").unwrap_or_else(|_| {
        "https://www.yujianqinghe.com/landing.html".to_string()
    })
}
...
async fn upload_file(bytes: &[u8], name: &str, mime: &str) -> Option<String> {
    ...
    // tmpfile 直链手机能开，优先；上传失败才转 litterbox（其下载被限速，只作兜底）
    match try_upload("https://tmpfile.link/api/upload", bytes, name, mime).await {
        Ok(url) => {
            log::info!("上传成功: {}", url);
            return Some(url);
        }
        Err(e) => log::warn!("上传 tmpfile.link 失败: {e}"),
    }
    match upload_litterbox(bytes, name).await {
        Ok(url) => {
            log::info!("上传成功: {}", url);
            return Some(url);
        }
        Err(e) => log::warn!("上传 litterbox 失败: {e}"),
    }
    log::error!("所有上传服务均失败");
    None
}

/// litterbox（catbox 临时版）：72h 直链，响应为纯文本 URL。
async fn upload_litterbox(bytes: &[u8], name: &str) -> anyhow::Result<String> {
    ...
    if !status.is_success() {
        anyhow::bail!("litterbox HTTP {} → {}", status, &body[..body.len().min(200)]);
    }
    let url = body.trim();
    if !url.starts_with("https://") {
        anyhow::bail!("litterbox 响应异常: {}", &body[..body.len().min(200)]);
    }
    Ok(url.to_string())
}
...
/// 上传 WAV + PNG，返回两个直链。任一文件读取失败只影响自己，不连累另一个。
async fn upload_share_assets(
    wav_path: &PathBuf,
    png_path: &PathBuf,
) -> (Option<String>, Option<String>) {
    let wav_bytes = tokio::fs::read(wav_path).await.ok().filter(|b| !b.is_empty());
    let png_bytes = tokio::fs::read(png_path).await.ok().filter(|b| !b.is_empty());

    let (wav_url, png_url) = tokio::join!(
        async {
            match wav_bytes {
                Some(b) => upload_file(&b, "music.wav", "audio/wav").await,
                None => None,
            }
        },
        async {
            match png_bytes {
                Some(b) => upload_file(&b, "art.png", "image/png").await,
                None => None,
            }
        },
    );
...
pub async fn create_share(data: &SharePageData) -> anyhow::Result<QrArtifact> {
    // 1. 上传 WAV + PNG，失败方留空 —— 文字 payload 始终编码进二维码
    let (wav_url, png_url) = upload_share_assets(&data.wav_path, &data.png_path).await;
    let audio_ok = wav_url.is_some() && png_url.is_some();
    let share_url = build_share_url(
        data,
        wav_url.as_deref().unwrap_or(""),
        png_url.as_deref().unwrap_or(""),
    );

...
// web 环境 fallback 到空态（无 mock）
const summary = ref<DailySummaryRow | null>(null)
const keyCountNow = ref(0) // 直读 key_events 表（新鲜计数）
const loading = ref(true)
const store = useAppStore()
const hourlyLevels = ref<number[]>([])
...
// hourly[24] → 热力图 level 1-4（最高值/4 为 level 4）

...
export function useTonePlayback(): TonePlayback {
  ...
  async function play(): Promise<void> {
    ...
    // v0.8.4: 支持暂停续播 —— 播完后再点播放从头开始；否则从 currentMs 对应 offset 续播。
    if (durationMs.value > 0 && currentMs.value >= durationMs.value - 50) {
      currentMs.value = 0
    }
    const offsetSec = currentMs.value / 1000
    const startedAt = (Tone as any).now() - offsetSec
    player.start(undefined, offsetSec)
  ...
  // v0.8.4: 真暂停 —— 停声但保留 currentMs，再点播放从暂停处续播
  //（旧版只有 stop：归零重播，用户反馈「暂停等于重头再来」）。

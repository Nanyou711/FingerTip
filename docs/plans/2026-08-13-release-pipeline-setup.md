...
发布时若想指向不同资源仓库（如 `Nanyou711/FingerTip`），设这些 Secrets：
...
| `FINGERTIP_LANDING_PAGE_URL` | `https://www.yujianqinghe.com/landing.html`（腾讯云 COS 托管） | QR 码落地页 URL |
| `FINGERTIP_COS_SECRET_ID` | (空) | COS 媒体上传密钥 ID（构建时注入；缺省则媒体上传失败、分享页降级文字版） |
| `FINGERTIP_COS_SECRET_KEY` | (空) | COS 媒体上传密钥 Key（同上） |
| `FINGERTIP_COS_BUCKET` | `fingertip-1468434404` | COS 桶（含 APPID） |
| `FINGERTIP_COS_REGION` | `ap-nanjing` | COS 桶地域 |
| `FINGERTIP_COS_PUBLIC_BASE` | `https://www.yujianqinghe.com` | 媒体公开访问域名 |
